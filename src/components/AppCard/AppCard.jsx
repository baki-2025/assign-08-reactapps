...
const AppCard = ({ singleApp }) => {
    const { id, image, title, rating, downloads } = singleApp;
    const navigate = useNavigate();

    const handleClick = () => {
        navigate(`/apps/${id}`); // navigate to AppDetails page
    };

    return (
        <div
            onClick={handleClick}
            className="cursor-pointer bg-white p-5 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 flex flex-col justify-between h-full relative"
        >
            {/* Top: Image + Title */}
            <div className="flex items-center gap-4 mb-4">
                <img
